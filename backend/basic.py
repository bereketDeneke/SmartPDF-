...
def INIT(inputImage, type, LANGUAGE, PDF:PDFWriter):  
    try:  
        pdf = PDF.getPdf()
        img, custom_config = config(inputImage, pdf, type, LANGUAGE)
        ...
        PDF.drawOnCanvas(Info, inputImage, height, width)
    except Exception as e:
        raise Exception(f"Basic.py Error {str(e)}")
